...
>
> **完成状态（已归档）：** 计划已全部闭环归档。
> - **Wave 1（止血与通知）：** 反馈改为登录后才能提交（FBK-002，不做链接过滤，未登录走论坛兜底；表单自动带真实用户名），结合 Resend 邮件基建实现新反馈实时邮件通知站长（FBK-001），已于 2026-09-21 实施并上线验证（app `57a1e86`）。
> - **Wave 2 及后续：** 用户已拍板跳过 Wave 2，FBK-003（后台状态列与倒序）不做、标为 `cancelled`（免去 alembic 数据库迁移与维护负担）；FBK-004（蜜罐/分类等免登录防滥用）此前已确认取消；待决策事项全部收敛关闭。
...
- 后台里能分辨**哪些反馈还没处理**、按时间倒序看最新的（~~FBK-003~~ 已取消，保持轻量运维）。
- 上述改动**不破坏现有反馈数据**，且发信 / 通知失败都不影响用户正常提交；登不上的用户能在页面上看到论坛兜底入口。

## 2. 这份计划怎么维护

沿用 [password-reset-plan.md](password-reset-plan.md)、[afdian-integration-plan.md](afdian-integration-plan.md) 的同一套约定：个人兴趣项目，不需要 owner / RACI。事项通常由一个 AI 工具推进，另一个 AI 工具独立检查；涉及站外账号（邮件服务商、DNS）或会改动生产数据的操作，由用户最后确认并执行。
...
### Wave 2：后台处理体验（P2，已取消）

范围：FBK-003（`cancelled`）。登录门槛上线后 spam 已近乎清零，真反馈直接通过邮件通知查看处理，不再需要给数据库加状态列或修改后台。原草案的 FBK-004（蜜罐 / 分类 / 匿名联系方式等表单增强）也已取消（`cancelled`）。
...
为何取消：2026-09-21 用户拍板不做。随着 FBK-002（登录门槛）上线，自动化冷推广 spam 已近乎清零，且 FBK-001（邮件实时通知）已让站长能在第一手邮件中查看和处理有效反馈。给 `feedback` 表新增状态列需要一次 alembic 数据库迁移，鉴于实际真反馈量较小，增加数据库列和后台维护逻辑并不划算，故跳过并不再实施。

重看触发：若未来战报网真实用户反馈量显著增大、邮件归档难以支撑，且确实需要团队多人跟进处理状态时，再重新评估。
...
- ✅ **落库改动接受度** → **全程零迁移**（身份复用现有 `username` 列；FBK-003 状态列取消，免去数据库改动）。原待决策 #1。
- ✅ **匿名联系方式 / 分类 / 蜜罐** → 登录门槛后**不做**（FBK-004 `cancelled`）。原待决策 #3、#5 的匿名回访诉求随之消解。
- ✅ **链接拦截**（2026-09-21）→ **不做**。是登录用户发的反馈就足够，不再限制内容是否含链接。原待决策 #2。
- ✅ **通知收件人** → 已配置（2026-09-21，ohmywod-ops `vars.yml` `feedback_notify_to`），生产验证通过。
- ✅ **是否顺带存 `user_id`（外键）** → **先不做**。现有 `username` 文本已足够定位到人，免去数据库外键迁移。
- ✅ **是否要“回复用户”能力** → **暂不做站内回复系统**。保持低维护边界，如有需要站长可通过邮箱单独联系。
- ✅ **是否推进 Wave 2（FBK-003）** → **不做**（FBK-003 `cancelled`）。邮件通知已满足跟进需求，不加状态列。
...
### 2026-09-21 — 计划归档（FBK-003 取消，文档归档）

- Drive AI：Antigravity；Review AI：未安排。
- 关联事项：FBK-003 `todo → cancelled`；`document_status` `active → done`。计划归档至 `docs/archive/feedback-improvement-plan.md`。
- 改动：Wave 1（FBK-001 邮件通知、FBK-002 登录门槛）已全面落地并验证；用户拍板跳过 Wave 2，取消 FBK-003（不修改后台或数据库加列），所有待决策项全部收敛闭环。更新文档元数据与归档相对链接。
- 关键取舍：不增加数据库迁移与后台管理复杂度，反馈防滥用与及时通知目标已完全达成，保持轻量免维护架构。
- 验证：全仓测试 164 项通过；文档内部链接校验通过。
- 发生的问题：无。
- 剩余风险：无。
- 下一步：无（计划已闭环归档）。
