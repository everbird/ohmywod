...
基于对代码库的通读整理，覆盖广告方案（见 `ad-optimization-plan-2026-07.md`）之外的改进方向，按优先级排序。截至撰写时**均未实施**。
...
**关于取真实 IP**：不必上 nginx realip 模块——限流 key 直接读 `CF-Connecting-IP` 头即可（HA 附录 A 的做法）。前提是这个头可信：HA 阶段 0 待办里的 Linode Cloud Firewall 把入站流量限到 Cloudflare IP 段之后，头才不可伪造。**这两件事要一起看**：Cloud Firewall 不只是安全兜底，也是限流生效的前提。nginx 日志想看真实 IP 再考虑 realip，属可选。
...
本项原本的主体（备份自动化、uptime 监控）已被 HA 计划阶段 0 完整覆盖且方案更好（Litestream 秒级 RPO 替代 cron `.backup`、restic、Healthchecks.io、UptimeRobot），**以 ha-plan.md 为准，此处不再重复**。

代码侧剩一件 ha-plan 没覆盖的事：

- **加 `/healthz` 路由**（检查 db、redis、数据挂载点可达），给 UptimeRobot 监控这个端点而不是首页。理由：首页不触碰数据目录（`disk_info` 那段有缓存且全 try/except 包着），JuiceFS 挂载悄悄掉线时首页照常 200，UptimeRobot 看不出异常——而代码里到处在防御挂载掉线，说明这是真实发生过的故障模式。HA 阶段 3 的切换脚本里的"冒烟检查"也可以直接复用这个端点。
...
1. 第 1 项（CSP sandbox，一行头）+ 第 2 项（bug 修复）——便宜且纯收益，与 HA 计划零冲突，先做。
2. 第 5 项（SEO）赶在广告方案上线前后完成——与流量增长是同一件事的两面，也不依赖任何 HA 阶段。
3. 第 4 项的 `busy_timeout` 随 HA 阶段 0 的 Litestream 接入一起上；第 6 项的 `/healthz` 也放进阶段 0（UptimeRobot 配好前就绪最好）。
