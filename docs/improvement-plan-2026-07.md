...
>
> **当前结论：最初识别的同源上传 HTML、常见 500、计数漂移、SEO 和 `/healthz` 已落地并部署到生产。跨仓主线现调整为 HA-008：先把认证从 LDAP 迁入 SQLite，再做单机 DR。本文不重复认证迁移状态；IMP-007 应尽早为迁移建立回归门槛，IMP-006 的登录/注册限流与 HA-008 同波完成，IMP-003 可在清理生产 LDAP 配置时并行收敛。缓存与搜索优化仍只在有实际性能证据时推进。**
