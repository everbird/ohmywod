...
## 明文镜像（rstore/reports/）
旧流程用 `juicefs sync` 往桶 `rstore/reports/` 存了一份 87.5GiB 明文可读镜像（现随迁移已在 Linode）。
本套**不再维护它**（新上传只走 JuiceFS chunks）。是否保留那份历史镜像作可读备份，由你决定；
如保留，需另写一个指向 Linode 的定期 `juicefs sync` 任务（不要用旧脚本里的 AWS 硬编码）。
